...
impl<'a> FunctionLowerer<'a> {
    ...
    fn lower_expression(&mut self, expression: &hir::Expr) -> Option<Operand> {
        match &expression.kind {
            ...
            hir::ExprKind::Lambda(_) => {
                self.diagnostics.push(
                    Diagnostic::error("lambda expressions are not supported by MIR lowering yet")
                        .with_code("E5004")
                        .at(expression.span),
                );
                None
    ...
    fn lower_argument_groups(&mut self, groups: &[Vec<hir::Expr>]) -> Option<Vec<Vec<Operand>>> {
        groups
            .iter()
            .map(|group| {
                group
    ...
    fn unsupported_statement(&mut self, statement: &hir::Stmt) {
        self.diagnostics.push(
            Diagnostic::error("HIR statement is not supported by MIR lowering yet")
                .with_code("E5007")
                .at(statement.span),
        );
    }
